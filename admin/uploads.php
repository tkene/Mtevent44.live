...
function img_load($id_produit)
{
    ...
    if (!empty(array_filter($_FILES['photos']['name']))) {
        foreach ($_FILES['photos']['name'] as $key => $val) {
            ...
            if (in_array($fileType, $allowTypes)) {
                ...
                if (move_uploaded_file($_FILES["photos"]["tmp_name"][$key], $targetFilePath)) {
                    // Image db insert sql
                    $insertValuesSQL .= "('" . $fileName . "', NOW(), $id_produit),";
                } else {
        ...
        $sql_img = "SELECT * FROM images where id_produit = $id_produit ";
        ...
        if ($resultat) {
            $sql_img = "DELETE FROM images WHERE id_produit = $id_produit";
            $sth = $connection->prepare($sql_img);
            $sth->execute();
        }
        ...
        if (!empty($insertValuesSQL)) {
            ...
            $insert = $connection->query("INSERT INTO images (name_photo, date_creat, id_produit) VALUES $insertValuesSQL");
